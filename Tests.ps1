...
function Get-UntisDataFromExport {
    ...
    $nr_of_columns = ((Get-Content "$Path/$filename" -Encoding 1252 | Select-Object -First 1).Split(',') | Measure-Object).Count
    ...
    $data = Get-Content "$Path/$filename" -Encoding 1252 | ConvertFrom-Csv -Header $colnames
    return $data
}
